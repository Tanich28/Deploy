...
async def read_only_mode(message: types.Message):
    ...
    #except Exception as err:
     #await message.answer(f"{err.__class__.name}: {err}")
    except BadRequest:
...
async def undban_user(message: types.Message):
    member = message.reply_to_message.from_user
    member_id = member.id
    chat_id = message.chat.id
    await message.chat.unban(user_id=member_id)
    await message.reply(f'Пользователь {member.full_name} был разабанен')
